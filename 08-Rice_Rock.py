...
def group_collide(sprite_group, other_sprite):
    ...
    for sprite in sprite_group:
        if sprite.collide(other_sprite):
            remove_set.add(sprite)
            exp_group.add(Sprite(sprite.get_position(), sprite.get_velocity(), 0, 0, explosion_image, explosion_info, explosion_sound))
    if remove_set:
        sprite_group.difference_update(remove_set)
...
def draw(canvas):
    global my_ship, best_score, score, lives, time, started, rock_group, exp_group, missiles_group
    ...
    # Draw UI
    # =======
    canvas.draw_text("Lives", [50, 50], 32, "Red")
    ...
    # ship - rocks collides
    collide_num = group_collide(rock_group, my_ship)
    if collide_num > 0:
        lives -= collide_num
    if lives <= 0:
        if best_score < score:
...
def rock_spawner():
    ...
    if started and len(rock_group) < 12 and dist(rock_pos, my_ship.get_position()) > 150:
